...
describe("filesystemBackedImpostersRepository", function () {
    ...
    describe("#add", function () {
        ...
        it("should add stubs for the imposter", async function () {
            ...
            const meta = read(
                `.mbtest/1000/${saved.stubs[0].meta.dir}/meta.json`
            );
            assert.strictEqual(meta.responseFiles.length, 1);

            const response = read(
                `.mbtest/1000/${saved.stubs[0].meta.dir}/${meta.responseFiles[0]}`
            );
    ...
    describe("#all", function () {
        it("should not retrieve imposters in database that have not been added", async function () {
            ...
            write(".mbtest/2000/imposter.json", {
                port: 2000,
                protocol: "test",
            });
            ...
            assert.deepEqual(stripFunctions(all), [
                { port: 1000, protocol: "test", stubs: [] },
            ]);
        });
    });
    ...
    describe("#del", function () {
        ...
        describe("#deleteAll", function () {
            ...
            it("does not remove any files not referenced by the repository", async function () {
                ...
                assert.strictEqual(
                    fs.existsSync(".mbtest/2000/imposter.json"),
                    true
                );
            });
        });
        ...
        describe("#stubsFor", function () {
            describe("#add", function () {
                it("should merge stubs with imposter header information", async function () {
                    ...
                    assert.deepEqual(
                        read(`.mbtest/3000/${stubDir}/${responseFile}`),
                        { is: { field: "response" } }
                    );
                });
                ...
                it("should add to stubs file if it already exists", async function () {
                    ...
                    assert.deepEqual(header, {
                        ...
                        stubs: [
                            {
                                predicates: [
                                    { equals: { field: "request-0" } },
                                ],
                                meta: { dir: stubDirs[0] },
                            },
                            {
                                predicates: [
                                    { equals: { field: "request-1" } },
                                ],
                                meta: { dir: stubDirs[1] },
                            },
                        ],
                ...
                it("should save multiple responses in separate files", async function () {
                    ...
                    assert.deepEqual(
                        read(`.mbtest/3000/${stubDir}/${responseFiles[0]}`),
                        { is: { field: "first-response" } }
                    );
                    assert.deepEqual(
                        read(`.mbtest/3000/${stubDir}/${responseFiles[1]}`),
                        { is: { field: "second-response" } }
                    );
                });
            ...
            describe("#insertAtIndex", function () {
                ...
                it("should add to stubs file if it already exists", async function () {
                    const stubs = repo.stubsFor(3000),
                        first = {
                            predicates: [
                                { equals: { field: "first-request" } },
                            ],
                            responses: [{ is: { field: "first-response" } }],
                        },
                        second = {
                            predicates: [
                                { equals: { field: "second-request" } },
                            ],
                    ...
                    const meta = read(`.mbtest/3000/${stubDirs[1]}/meta.json`),
                        responseFile = meta.responseFiles[0];
                    assert.deepEqual(
                        read(`.mbtest/3000/${stubDirs[1]}/${responseFile}`),
            ...
            describe("#deleteAtIndex", function () {
                it("should delete stub and stub dir at specified index", async function () {
                    const stubs = repo.stubsFor(3000),
                        ...
                        third = {
                            predicates: [
                                { equals: { field: "third-request" } },
                            ],
                            responses: [{ is: { field: "third-response" } }],
                        },
                        imposter = {
                            port: 3000,
                            stubs: [first, second, third],
                        };
                    ...
                    assert.ok(
                        !fs.existsSync(`.mbtest/3000/${stubDirToDelete}`)
                    );
                });
            });
            ...
            describe("#toJSON", function () {
                ...
                it("should error if database corrupted via corrupted JSON", async function () {
                    ...
                    try {
                        ...
                        // Node.js changed JSON.parse error messages across versions.
                        // Accept both legacy (Node <=18) and newer (Node >=20) formats.
                        assert.strictEqual(err.code, "corrupted database");
                        assert.strictEqual(
                            err.message,
                            `invalid JSON in ${responsePath}`
                        );
                        assert.match(
                            err.details,
                            /(Unexpected token C in JSON at position 0|Unexpected token 'C',\s+"CORRUPTED" is not valid JSON)/,
                            `Unexpected details format: ${err.details}`
                        );
                    }
                });
            });
