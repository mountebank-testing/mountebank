        ...
        describe('POST /imposters with stubs', function () {
            ...
            it('should support shell transform without array for backwards compatibility', async function () {
                ...
                const stub = {
                        responses: [{
                            is: { body: 'Hello, {YOU}!' },
                            _behaviors: { shellTransform: `node ${process.cwd()}/shellTransformTest.js` }
                        }]
                    },
                    stubs = [stub],
                    imposter = { protocol, port, stubs: stubs },
            ...
            it('should support array of shell transforms in order', async function () {
                const stub = {
                        responses: [{
                            is: { body: 1 },
                            _behaviors: {
                                shellTransform: [`node ${process.cwd()}/double.js`, `node ${process.cwd()}/increment.js`]
                            }
                        }]
                    },
                    stubs = [stub],
                                ...
                                lookup: [
                                    {
                                        ...
                                        fromDataSource: { csv: { path: `${process.cwd()}/lookupTest.csv`, keyColumn: 'name' } },
                                        into: '${mountebank}'
                                    },
                                    {
                                        key: { from: { headers: 'X-Bob' }, using: { method: 'regex', selector: '.+' } },
                                        fromDataSource: { csv: { path: `${process.cwd()}/lookupTest.csv`, keyColumn: 'occupation' } },
                                        into: '${bob}'
                                    }
                                ]
                            }
